...
palStatus_t pal_imagePrepare(palImageId_t imageId, palImageHeaderDeails_t *headerDetails)
{
    ...
    if (ret == PAL_SUCCESS)
    {
        ...
        if (NULL != buffer)
        {
        	uint32_t writeCounter = headerDetails->imageSize;
			memset(buffer,0,PAL_KILOBYTE);
			while(writeCounter + PAL_KILOBYTE <= headerDetails->imageSize)
			{
				ret = safe_write(imageId,0,buffer,PAL_KILOBYTE);
				if (PAL_SUCCESS != ret)
				{
					break;
				}
			...
			if ((PAL_SUCCESS == ret) && (writeCounter < headerDetails->imageSize))
			{
				...
				ret = safe_write(imageId,0,buffer,(headerDetails->imageSize - writeCounter));
			}
			free(buffer);
			if (PAL_SUCCESS == ret)
			{
