...
void palConnectCallBack()
{
        ...
        palStatus_t PALSocketWrapper::setRxBuffer( char data)
        {
            if (true == rxBufferSet)
            {
                return PAL_ERR_SOCKET_GENERIC; // should never overwrite buffer.
            }
            rxBuffer = data;
            rxBufferSet = true;
        ...
        // check that we got a valid socket and the socket type is supported
            if ((true == initialized) || (NULL == socket) || ((socketType != PAL_SOCK_STREAM) && (socketType != PAL_SOCK_STREAM_SERVER) && (socketType != PAL_SOCK_DGRAM)))
            {
        ...
        nsapi_size_or_error_t status = NSAPI_ERROR_OK;
            if ((false == initialized) || (PAL_SOCK_DGRAM != socketTypeVal)) // udp sockets only
            {
                return NSAPI_ERROR_PARAMETER;
            }
        ...
        {
        nsapi_error_t status = NSAPI_ERROR_OK;
            if ((false == initialized) || (PAL_SOCK_STREAM != socketTypeVal)) // tcp sockets only
            {
                return NSAPI_ERROR_PARAMETER;
            }
        ...
        {
        nsapi_error_t status = NSAPI_ERROR_OK;
            if ((false == initialized) || (PAL_SOCK_STREAM_SERVER != socketTypeVal)) // udp sockets only
            {
                return NSAPI_ERROR_PARAMETER;
            }
    ...
    uint8_t palSocketStatus[PAL_NET_SOCKET_SELECT_MAX_SOCKETS], uint32_t * numberOfSocketsSet)
{
        ...
        if (0 == numberOfSockets) // if no sockets to check return immediately
        {
            return PAL_SUCCESS;
        }

        for (index = 0; index < numberOfSockets; index++) // check that sockets given are valid
        {
            if (NULL == socketsToCheck[index])
            {
                return PAL_ERR_INVALID_ARGUMENT;
            }
        }
         ...
         *numberOfSocketsSet = 0;
         ...
         for (uint32_t index = 0; index < numberOfSockets; index++) // clear socekt data
         {
             s_select_event_happened[index] = 0;
             palSocketStatus[index] = 0;
         }

         for (index = 0; index < numberOfSockets; index++) // check if any of the sockets have data ready to read , if so set setatus and return immediately.
         {
             bool isNonBlocking = false;
             char tmpBuffer[1];
             nsapi_size_or_error_t received = 0;

             PALSocketWrapper* socketObj = (PALSocketWrapper*)socketsToCheck[index];

             if (true == socketObj->isRxBufferSet()) // check if socket already has an RX buffer set
             {
                 countWithPriorData++;
                 palSocketStatus[index] |= PAL_NET_SOCKET_SELECT_RX_BIT | PAL_NET_SOCKET_SELECT_TX_BIT;
             }
             else { // otherwise set socket to non-blovking and try to read data (save data in socket buffer for actual reading later).
                 if (PAL_SOCK_DGRAM == socketObj->getSocketType())
                 {
                     isNonBlocking = socketObj->isNonBlocking();
                     socketObj->set_blocking(false);
                     received = socketObj->recvfrom(NULL, tmpBuffer, 1);
                     if (NSAPI_ERROR_WOULD_BLOCK != received)
                     {
                         if (received > 0)
                         {
                             socketObj->setRxBuffer(tmpBuffer[0]);
                         }
                         countWithPriorData++;
                         palSocketStatus[index] |= PAL_NET_SOCKET_SELECT_RX_BIT;
                     }
                     socketObj->set_blocking(!isNonBlocking);
                 }
                 else if (PAL_SOCK_STREAM == socketObj->getSocketType())
                 {
                     isNonBlocking = socketObj->isNonBlocking();
                     socketObj->set_blocking(false);
                     received = socketObj->recv(tmpBuffer, 1);
                     if ((0 < received) || (true == socketObj->isConnected()))
                     {
                         countWithPriorData++;
                         if (true == socketObj->isConnected())
                         {
                             palSocketStatus[index] |= PAL_NET_SOCKET_SELECT_TX_BIT;
                         }
                         if (0 < received)
                         {
                             socketObj->setRxBuffer(tmpBuffer[0]);
                 ...
                 else if (PAL_SOCK_STREAM_SERVER == socketObj->getSocketType())
                 {
                     // no actual data is exepcted to be recieved on a server socket.
                 }
                 else
                 {
                     result = PAL_ERR_INVALID_ARGUMENT; // should never happen
                     goto finish;
                 }
             }
         }
         if (countWithPriorData > 0) // some sockets have data ready to read - no need to block just return value.
         {
             *numberOfSocketsSet = countWithPriorData;
             result = PAL_SUCCESS;
             goto finish;
         }

         for (index = 0; index < numberOfSockets; index++) // set callbacks to wake us up during blocking
         {
             PALSocketWrapper* socketObj = (PALSocketWrapper*)socketsToCheck[index];
             socketObj->updateCallback(s_palSelectPalCallbackFunctions[index]);
         }

         result = pal_osSemaphoreWait(s_palSelectSemaphore, timeoutInMiliseconds, &counter); // wait on semaphore with count 0 till timeout happlens or a callback is called.
         if (result == PAL_SUCCESS)
         {
             for (index = 0; index < numberOfSockets; index++)
             {
                 if (s_select_event_happened[index] > 0) //if socket was set modify socket status accordingly.
                 {
         ...
         for (index = 0; index < numberOfSockets; index++) // reset socket callbacks to normal.
         {

             PALSocketWrapper* socketObj = (PALSocketWrapper*)socketsToCheck[index];
             socketObj->updateCallback(NULL);
         }
finish:
         return result ;
}
