...
// Thread function.
static void* asyncSocketManager(void *arg)
{
...
palStatus_t pal_plat_socketsInit(void* context)
{
    ...
    pthread_attr_setstacksize (&attr, PAL_NET_TEST_ASYNC_SOCKET_MANAGER_THREAD_STACK_SIZE);  //sets the minimum stack size
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);

    // create the thread
    res = pthread_create(&s_pollThread, &attr, &asyncSocketManager, 0);
    if (res < 0)
    {
...
palStatus_t pal_plat_socketsTerminate(void* context)
{
    ...
    pthread_join(s_pollThread, NULL);
...
palStatus_t pal_plat_socket(palSocketDomain_t domain, palSocketType_t type, bool nonBlockingSocket, uint32_t interfaceNum, palSocket_t* sockt)
{
    ...
    if (interfaceNum >= s_palNumOfInterfaces && PAL_NET_DEFAULT_INTERFACE != interfaceNum)
    {
...
palStatus_t pal_plat_getSocketOptions(palSocket_t socket, palSocketOptionName_t optionName, void* optionValue, palSocketLength_t* optionLength)
{
    ...
    // TODO moved to shared function
    switch ((palSocketOptionName_t)optionName)
    {
    ...
    if (result == PAL_SUCCESS)
    {
        if (PAL_SO_REUSEADDR == optionName ||
            PAL_SO_SNDTIMEO == optionName ||
            PAL_SO_RCVTIMEO == optionName)
        {
            result = getsockopt ((int)socket, SOL_SOCKET, linuxName, optionValue, optionLength);
        }
#ifdef PAL_NET_TCP_AND_TLS_SUPPORT
        else if (PAL_SO_KEEPALIVE == optionName)
        {
            result = getsockopt((int)socket, SOL_SOCKET, linuxName, optionValue, optionLength);
        }
        else
        {
            result = getsockopt((int)socket, SOL_TCP, linuxName, optionValue, optionLength);
        }
#endif
...
palStatus_t pal_plat_setSocketOptions(palSocket_t socket, int optionName, const void* optionValue, palSocketLength_t optionLength)
{
    ...
    if (PAL_SUCCESS == result)
    {
        ...
        {
            result = setsockopt ((int)socket, SOL_SOCKET, linuxName, &timeout, sizeof(timeout));
        }
#ifdef PAL_NET_TCP_AND_TLS_SUPPORT
        else if (PAL_SO_KEEPIDLE == optionName || PAL_SO_KEEPINTVL == optionName)
        {
            result = setsockopt ((int)socket, SOL_TCP, linuxName, (int *)optionValue, optionLength);
        }
#endif
        else
        {
            result = setsockopt ((int)socket, SOL_SOCKET, linuxName, (int *)optionValue, optionLength);
        }
...
palStatus_t pal_plat_bind(palSocket_t socket, palSocketAddress_t* myAddress, palSocketLength_t addressLength)
{
    ...
    if (result == PAL_SUCCESS)
    {
        res = bind((int)socket, (struct sockaddr *)&internalAddr, addressLength);
        if (res == -1)
        {
            result = translateErrorToPALError(errno);
        }
...
palStatus_t pal_plat_receiveFrom(palSocket_t socket, void* buffer, size_t length, palSocketAddress_t* from, palSocketLength_t* fromLength, size_t* bytesReceived)
{
    ...
    clearSocketFilter((int)socket);
    addrlen = sizeof(struct sockaddr_storage);
    res = recvfrom((int)socket, buffer, length, 0 ,(struct sockaddr *)&internalAddr, &addrlen);
    if(res == -1)
    {
        result = translateErrorToPALError(errno);
    }
...
palStatus_t pal_plat_sendTo(palSocket_t socket, const void* buffer, size_t length, const palSocketAddress_t* to, palSocketLength_t toLength, size_t* bytesSent)
{
    ...
    clearSocketFilter((int)socket);
    res = sendto((int)socket, buffer, length, 0, (struct sockaddr *)to, toLength);
    if(res == -1)
    {
        result = translateErrorToPALError(errno);
    }
...
palStatus_t pal_plat_getNetInterfaceInfo(uint32_t interfaceNum, palNetInterfaceInfo_t * interfaceInfo)
{
    ...
    if (interfaceNum >= s_palNumOfInterfaces)
    {
        return PAL_ERR_INVALID_ARGUMENT;
    }
    ...
    uint8_t palSocketStatus[PAL_NET_SOCKET_SELECT_MAX_SOCKETS], uint32_t * numberOfSocketsSet)
{
        ...
        // Keep a local copy because select in Linux will modify it.
        ...
        // Copy the values manually as the struct variables may have different sizes.
        tmpTimeout.tv_sec = timeout->pal_tv_sec;
        tmpTimeout.tv_usec = timeout->pal_tv_usec;
        ...
        *numberOfSocketsSet = 0;
        ...
        {
            fd = (int)socketsToCheck[index];
            FD_SET(fd, &readset);
            // Do not detect writes because they will not block unless something is in the process of being written(Nir)
            // XXX: Actually for a nonblocking connect() we need to detect readiness for write
        ...
        res = select(maxfd + 1, &readset, &writeset, &exset, &tmpTimeout);
        if(res == -1)
        {
            result = translateErrorToPALError(errno);
        }
        else if (res > 0) // select returns 0 in case timeout expire
        {
            for(index = 0; index < numberOfSockets; index++)
            {
                ...
                {
                    socketStatus |= PAL_NET_SOCKET_SELECT_RX_BIT;
                }
                if(FD_ISSET(fd, &writeset))
                {
                    socketStatus |= PAL_NET_SOCKET_SELECT_TX_BIT;
                }
                if(FD_ISSET(fd, &exset))
                {
                    socketStatus |= PAL_NET_SOCKET_SELECT_ERR_BIT;
                }

                palSocketStatus[index] = socketStatus;
            }
            *numberOfSocketsSet = res;
        }
...
palStatus_t pal_plat_accept(palSocket_t socket, palSocketAddress_t * address, palSocketLength_t* addressLen, palSocket_t* acceptedSocket)
{
    ...
    res = accept((int)socket,(struct sockaddr *)&internalAddr, addressLen);
    if(res == -1)
    {
        result = translateErrorToPALError(errno);
    }
...
palStatus_t pal_plat_connect(palSocket_t socket, const palSocketAddress_t* address, palSocketLength_t addressLen)
{
    ...
    if (result == PAL_SUCCESS)
    {
        res = connect((int)socket,(struct sockaddr *)&internalAddr, addressLen);
        if(res == -1)
        {
            result = translateErrorToPALError(errno);
        }
...
palStatus_t pal_plat_recv(palSocket_t socket, void *buffer, size_t len, size_t* recievedDataSize)
{
    ...
    clearSocketFilter((int)socket);
    res = recv((int)socket, buffer, len, 0);
    if(res ==  -1)
    {
        result = translateErrorToPALError(errno);
    }
...
palStatus_t pal_plat_asynchronousSocket(palSocketDomain_t domain, palSocketType_t type, bool nonBlockingSocket, uint32_t interfaceNum, palAsyncSocketCallback_t callback, void* callbackArgument, palSocket_t* socket)
{
    ...
    palStatus_t result = pal_plat_socket(domain,  type,  nonBlockingSocket,  interfaceNum, socket);
        ...
        err = fcntl((int)*socket, F_SETOWN, getpid());
        assert(err != -1);
        ...
        err = fcntl((int)*socket, F_SETFL, flags);
