...
PAL_PRIVATE palStatus_t pal_plat_X509GetField(palX509Ctx_t* x509Ctx, const char* fieldName, void* output, size_t outLenBytes, size_t* actualOutLenBytes)
{
    ...
    if (NULL == fieldName) // other arguments already checked in service layer.
    {
...
PAL_PRIVATE palStatus_t pal_timegm( struct tm *tm, uint64_t* outTime) 
{
    ...
    if (NULL == outTime || tm->tm_year < 1970)
    {
        return PAL_ERR_INVALID_ARGUMENT;
    }
...
palStatus_t pal_plat_x509CertVerify(palX509Handle_t x509Cert, palX509Handle_t x509CertChain)
{
    palStatus_t status = PAL_SUCCESS;
    palX509Ctx_t* localCert = (palX509Ctx_t*)x509Cert;
    palX509Ctx_t* localCAChain = (palX509Ctx_t*)x509CertChain;
    //mbedtls_x509_crt_profile profiles = {1,1,1,1};
    int32_t platStatus = CRYPTO_PLAT_SUCCESS;
    uint32_t flags = 0;

    if (NULL == localCAChain)
    {
        //platStatus = mbedtls_x509_crt_verify(&localCert->crt, NULL, NULL, NULL, &flags, NULL, NULL);
        platStatus = mbedtls_x509_crt_verify_with_profile(&localCert->crt, NULL, NULL, &s_PALProfile, NULL, &flags, NULL, NULL);
    }
    else
    {
        //platStatus = mbedtls_x509_crt_verify(&localCert->crt, &localCAChain->crt, NULL, NULL, &flags, NULL, NULL);
...
palStatus_t pal_plat_CtrDRBGFree(palCtrDrbgCtxHandle_t* ctx)
{
    ...
    palCtrDrbgCtx = (palCtrDrbgCtx_t*)*ctx;
    if (NULL == palCtrDrbgCtx)
    {
        return PAL_ERR_INVALID_ARGUMENT;
    }
...
palStatus_t pal_plat_CtrDRBGSeed(palCtrDrbgCtxHandle_t ctx, const void* seed, size_t len)
{
    ...
    platStatus = mbedtls_ctr_drbg_seed(&palCtrDrbgCtx->ctrDrbgCtx, pal_plat_entropySourceDRBG, &palCtrDrbgCtx->entropy, seed, len);
    if (CRYPTO_PLAT_SUCCESS != platStatus)
    {
        switch(platStatus)
        {
...
palStatus_t pal_plat_CtrDRBGGenerate(palCtrDrbgCtxHandle_t ctx, unsigned char* out, size_t len)
{
    ...
    platStatus = mbedtls_ctr_drbg_random(&palCtrDrbgCtx->ctrDrbgCtx, out, len);
    if (CRYPTO_PLAT_SUCCESS != platStatus)
    {
        switch(platStatus)
        {
...
palStatus_t pal_plat_CMACFinish(palCMACHandle_t *ctx, unsigned char *output, size_t* outLen)
{
    ...
    localCipher = (palCipherCtx_t*)*ctx;
    if (NULL == localCipher->cipher_info)
    {
        return PAL_ERR_INVALID_ARGUMENT;
    }
...
PAL_PRIVATE palStatus_t pal_plat_ECCheckPrivateKey(palECGroup_t* ecpGroup, palECKeyHandle_t key, bool *verified)
{
    ...
    if (NULL == ecpGroup || NULL == privateKey || NULL == verified || NULL == (mbedtls_ecp_keypair*)privateKey->pk_ctx)
    {
        return PAL_ERR_INVALID_ARGUMENT;
    }
...
PAL_PRIVATE palStatus_t pal_plat_ECCheckPublicKey(palECGroup_t* ecpGroup, palECKeyHandle_t key, bool *verified)
{
    ...
    if (NULL == ecpGroup || NULL == publicKey || NULL == verified || NULL == (mbedtls_ecp_keypair*)publicKey->pk_ctx)
    {
        return PAL_ERR_INVALID_ARGUMENT;
    }
...
PAL_PRIVATE int pal_plat_entropySourceDRBG( void *data, unsigned char *output, size_t len)
{
    ...
    //! We need to call platform layer function, in order to prevent two things:
    //! 1. adding new service layer function for this purpose (only for DRBG seeding for mbedTLS, not sure other Crypto libraries will need it)
    //! 2. prevent endless recursive (random->drbg->random) - reseeding scenario.
    status = pal_plat_osRandomBuffer((uint8_t*) output, len);
    if (PAL_SUCCESS == status)
    {
