 ...
 * | 8 | Release x509 certificate context using `pal_x509Free`.                                       | PAL_SUCCESS |
 * | 9 | Initialize X509 ceritifcate context using `pal_x509Initiate`.                                | PAL_SUCCESS |
 * | 10 | Parse a valid x509 certificate using `pal_x509CertParse`.                                   | PAL_SUCCESS |
 * | 11 | Get the certificate attribute value using `pal_x509CertGetAttribute` and check it.          | PAL_SUCCESS |
 ...
 * | 16 | Get the certificate attribute value with a too small buffer.                                | PAL_ERR_BUFFER_TOO_SMALL |
 * | 17 | Release x509 certificate context using `pal_x509Free`.                                      | PAL_SUCCESS |
...
TEST(pal_crypto, X509_Verify)
{
    ...
    for (size_t i = 0; i < sizeof(x509_verify_data) / sizeof(palX509VertifyTestVector_t); ++i) 
    {
        ...
        TEST_ASSERT_EQUAL_HEX(PAL_SUCCESS, result);
        /*#5*/
        result = pal_x509CertVerify(cert, caCert);
