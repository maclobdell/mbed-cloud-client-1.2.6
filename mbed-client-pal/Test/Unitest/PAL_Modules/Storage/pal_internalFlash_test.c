...
TEST(pal_internalFlash, BasicTest)
{
	...
	DEBUG_PRINT("---------FULL SECTOR TEST---------r\n\r");
    for (uint32_t i = 0; i < biggestSectorSize / 4; i++)
    {
    	compareBuffer[biggestSectorSize - i - 1] = (uint8_t)(i % 256);
    }
...
* | 2 | Write data that crosses between sectors      						| PAL_ERR_INTERNAL_FLASH_CROSSING_SECTORS |
* | 3 | Write with null ptr has buffer					                    | PAL_ERR_INTERNAL_FLASH_CROSSING_SECTORS |
* | 4 | write with address not align to page size               		    | PAL_ERR_INTERNAL_FLASH_ADDRESS_NOT_ALIGNED |
* | 5 | write to unaligned buffer  								            | PAL_ERR_INTERNAL_FLASH_BUFFER_ADDRESS_NOT_ALIGNED |
*/
...
TEST(pal_internalFlash, NegativeTest)
{
    ...
    /*2*/
	status = pal_internalFlashWrite(areaOneData.size * 2, areaOneData.address, (uint32_t*)&ReadBuffer1);
	TEST_ASSERT_EQUAL_HEX(status, PAL_ERR_INTERNAL_FLASH_CROSSING_SECTORS);
