...
FtcdCommSocket::FtcdCommSocket(const void *interface_handler, ftcd_socket_domain_e domain, const uint16_t port_num, int32_t timeout)
{
    _interface_handler = interface_handler;
    _required_domain_type = domain;

    /* If port supplied is 0, generate random port similar to UNIX convention.
    *  Pal currently does not support binding with port 0.
    */
    _port = port_num;
    if (port_num == 0) {
        srand(pal_osKernelSysTick());

        // Generate random port int the range [RANDOM_PORT_MIN, RANDOM_PORT_MAX - 1] including.
        _port = (rand() % (RANDOM_PORT_MAX - RANDOM_PORT_MIN)) + RANDOM_PORT_MIN;
    }
...
{
    char expected_token[] = FTCD_MSG_HEADER_TOKEN;
    char c;
...
uint32_t FtcdCommSocket::read_message_size(void)
{
    ...
    if (result != FTCD_COMM_STATUS_SUCCESS) {
        mbed_tracef(TRACE_LEVEL_CMD, TRACE_GROUP, "Failed reading message size\r");
        return 0;
    }
...
bool FtcdCommSocket::read_message(uint8_t *message_out, size_t message_size)
{
    ...
    if (message_out == NULL) {
        mbed_tracef(TRACE_LEVEL_CMD, TRACE_GROUP, "Invalid message buffer\r");
        return false;
    }

    // Read CBOR message bytes
    // We assume that LENGTH is NOT bigger than INT_MAX
    result = _read_from_socket(reinterpret_cast<void*>(message_out), message_size);
    if (result != FTCD_COMM_STATUS_SUCCESS) {
        mbed_tracef(TRACE_LEVEL_CMD, TRACE_GROUP, "Failed reading message bytes\r");
        return false;
    }
    return true;
}
...
bool FtcdCommSocket::read_message_signature(uint8_t *sig, size_t sig_size)
{
    ...
    if (sig == NULL) {
        mbed_tracef(TRACE_LEVEL_CMD, TRACE_GROUP, "Invalid sig buffer\r");
        return false;
    }

    // Read signature from medium
    result = _read_from_socket(reinterpret_cast<void*>(sig), sig_size);
    if (result != FTCD_COMM_STATUS_SUCCESS) {
        mbed_tracef(TRACE_LEVEL_CMD, TRACE_GROUP, "Failed reading message signature bytes\r");
        return false;
    }
    return true;
}
...
bool FtcdCommSocket::send(const uint8_t *data, uint32_t data_size)
{
    ...
    do {
        ...
        if (result != PAL_SUCCESS) {
            mbed_tracef(TRACE_LEVEL_CMD, TRACE_GROUP, "Failed pal_send\r");
            success = false;
            break;
        }
...
ftcd_comm_status_e FtcdCommSocket::_read_from_socket(void * data_out, int data_out_size)
{
    ...
    int bytes_read_successfully = bytes_received;

    // The remaining bytes left to read
    int remaining_bytes = data_out_size - bytes_received;
