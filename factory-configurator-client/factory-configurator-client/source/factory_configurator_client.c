...
fcc_status_e fcc_entropy_set(const uint8_t *buf, size_t buf_size)
{
    ...
    fcc_status = fcc_sotp_data_store(buf, buf_size, FCC_SOTP_TYPE_ENTROPY);
...
fcc_status_e fcc_rot_set(const uint8_t *buf, size_t buf_size)
{
    ...
    fcc_status = fcc_sotp_data_store(buf, buf_size, FCC_SOTP_TYPE_ROT);
...
fcc_status_e fcc_secure_time_set(uint64_t time)
{
    ...
    //FIXME: Should be replaced by some other ESFS API that will handle situations where time is already set
    pal_status = pal_osSetTime(time);
...
fcc_status_e fcc_factory_disable(void)
{
    ...
    fcc_status = fcc_sotp_data_store((uint8_t *)(&factory_disable_flag), sizeof(factory_disable_flag), FCC_SOTP_TYPE_FACTORY_DISABLE);
...
fcc_status_e fcc_is_factory_disabled(bool *is_factory_disabled)
{
    ...
    fcc_status = fcc_sotp_data_retrieve((uint8_t *)(&factory_disable_flag), sizeof(factory_disable_flag), &data_actual_size_out, FCC_SOTP_TYPE_FACTORY_DISABLE);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((fcc_status != FCC_STATUS_SUCCESS), fcc_status, "Failed for fcc_sotp_buffer_retrieve");
