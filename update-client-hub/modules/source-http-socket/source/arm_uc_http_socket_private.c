...
arm_uc_error_t arm_uc_socket_connect()
{
    ...
    if (context && context->request_uri)
    {
        if (context->socket_state == STATE_DISCONNECTED)
        {
            ...
            palStatus_t pal_inner = pal_asynchronousSocket(PAL_AF_INET,
                                                           PAL_SOCK_STREAM,
                                                           true,
                                                           0,
                                                           arm_uc_socket_isr,
            ...
            /* convert URI to PAL address if cache is empty */
            if ((result.code == SRCE_ERR_NONE) &&
                (context->cache_address_length == 0))
            {
                /* dns loopup */
                pal_inner = pal_getAddressInfo(context->request_uri->host,
...
arm_uc_error_t arm_uc_socket_send_request()
{
    ...
    if (context)
    {
        ...
        if ((pal_result == PAL_SUCCESS) || (pal_result == PAL_ERR_SOCKET_WOULD_BLOCK)) /* asynchronous finish */
        {
