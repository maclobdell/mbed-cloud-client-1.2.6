...
/**
 * @brief return the index of the source with the smalllest cost
...
/**
 * @brief Find the source of lowest cost and call the correspoding method
                                                           ...
                                                           uint32_t* index)
{
    ...
    for (uint32_t i=0; i<MAX_SOURCES; i++)
    {
        ...
        else if (uri != NULL && type == QUERY_TYPE_FRIMWARE && cap.firmware == 1)
        {
...
static arm_uc_error_t ARM_UCSM_Get(request_t* req)
{
    ...
    else if ((req->uri != NULL) && (req->type == QUERY_TYPE_FRIMWARE))
    {
        UC_SRCE_TRACE("calling source %" PRIu32 " GetFirmwareFragment", index);
    ...
    else if (retval.error != ERR_NONE)
    {
        ...
        UC_SRCE_TRACE("failure, try source with the next smallest cost");
    ...
    UC_SRCE_TRACE("Current Source %" PRIu32, index);
                                            ...
                                            uint32_t offset)
{
    ...
    request_in_flight.buffer = buffer;
    request_in_flight.offset = offset;
    request_in_flight.type   = QUERY_TYPE_FRIMWARE;
