                                           ...
                                           arm_uc_buffer_t* buffer)
{
    ...
    {
        tr_debug("ARM_UC_PAL_FlashIAP_Prepare: %" PRIX32 " %" PRIX32,
        ...
        if (result.error == ERR_NONE)
        {
            ...
            {
                uint32_t sector_size = arm_uc_flashiap_get_sector_size(start_address);

                int32_t status = arm_uc_flashiap_erase(erase_address, sector_size);

                tr_debug("erase: %" PRIX32 " %" PRIX32 " %" PRId32,
                         erase_address,
                         sector_size,
                         status);
            ...
            if (result.error == ERR_NONE)
            {
                tr_debug("program: %" PRIX32 " %" PRIX32,
                         start_address,
                         ARM_UC_PAL_HEADER_SIZE);
                ...
                {
                    tr_error("arm_uc_flashiap_program failed");
                }
            }
            else
            {
                tr_error("arm_uc_flashiap_erase failed");
            }
        }
        else
        {
            tr_error("arm_uc_create_internal_header_v2 failed");
        }
    }
                                         ...
                                         const arm_uc_buffer_t* buffer)
{
    ...
    if (buffer && buffer->ptr)
    {
        ...
        tr_debug("ARM_UC_PAL_FlashIAP_Write: %p %" PRIX32 " %" PRIX32 " %" PRIX32,
                 buffer->ptr,
                 buffer->size,
                 physical_address,
                 offset);
...
arm_uc_error_t ARM_UC_PAL_FlashIAP_Finalize(uint32_t location)
{
    ...
    tr_debug("ARM_UC_PAL_FlashIAP_Finalize");
                                        ...
                                        arm_uc_buffer_t* buffer)
{
    ...
    if (buffer && buffer->ptr)
    {
        tr_debug("ARM_UC_PAL_FlashIAP_Read: %" PRIX32 " %" PRIX32 " %" PRIX32,
                 location, offset, buffer->size_max);
        ...
        {
            tr_error("arm_uc_flashiap_read failed");
        }
    }
...
arm_uc_error_t ARM_UC_PAL_FlashIAP_Activate(uint32_t location)
{
    ...
    tr_debug("ARM_UC_PAL_FlashIAP_Activate");
                                        ...
                                        arm_uc_firmware_details_t* details)
{
    ...
    if (details)
    {
        tr_debug("ARM_UC_PAL_FlashIAP_GetFirmwareDetails");
        ...
        if (status == ARM_UC_FLASHIAP_SUCCESS)
        {
            ...
            {
                tr_error("arm_uc_parse_internal_header_v2 failed");
            }
        }
        else
        {
            ...
            if (rc == ARM_UC_FLASHIAP_SUCCESS)
            {
                ...
                switch(headerVersion)
                {
                    case ARM_UC_INTERNAL_HEADER_VERSION_V2:
                    {
                        ...
                        {
                            tr_error("firmware header is v2, but does not contain v2 magic");
                            result.code = ERR_NOT_READY;
                        }
                        ...
                        if (result.error == ERR_NONE)
                        {
                            ...
                            {
                                result.code = ERR_NOT_READY;
                                tr_error("failed to read v2 header");
                            }
                        }
                        /* Parse the header */
                        if (result.error == ERR_NONE)
                        {
                            ...
                            if (result.error != ERR_NONE)
                            {
                                tr_error("failed to parse v2 header");
                            }
                        }
                        break;
                    }
                    ...
                    default:
                    {
                        tr_error("unrecognized firmware header version");
                        result.code = ERR_NOT_READY;
                    }
                }
            }
